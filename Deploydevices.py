...
def printhelp():
    ...
    printusertext(
        'python deploydevices.py -k <key> -o <org> -s <sn> -n <netw> -c <conft> [-t <tag> -a <addr> -g <gkey> -m ignore_error]')
    printusertext('')
...
def createnw(p_apikey, p_shardurl, p_dstorg, p_nwdata):
    ...
    if nwtype != 'systems manager':
        try:
            r = requests.post('https://%s/api/v0/organizations/%s/networks' % (p_shardurl, p_dstorg), data=json.dumps(
                {'timeZone': p_nwdata['timeZone'], 'tags': p_nwdata['tags'], 'name': p_nwdata['name'],
                 'organizationId': p_dstorg, 'type': nwtype}),
                              headers={'X-Cisco-Meraki-API-Key': p_apikey, 'Content-Type': 'application/json'})
        except:
...
def updatenw(p_apikey, p_shardhost, p_nwid, p_field, p_value):
    ...
    try:
        r = requests.put('https://%s/api/v0/networks/%s' % (p_shardhost, p_nwid), data=json.dumps({p_field: p_value}),
                         headers={'X-Cisco-Meraki-API-Key': p_apikey, 'Content-Type': 'application/json'})
    except:
...
def gettemplateid(p_apikey, p_shardurl, p_orgid, p_tname):
    ...
    try:
        r = requests.get('https://%s/api/v0/organizations/%s/configTemplates' % (p_shardurl, p_orgid),
                         headers={'X-Cisco-Meraki-API-Key': p_apikey, 'Content-Type': 'application/json'})
    except:
...
def bindnw(p_apikey, p_shardurl, p_nwid, p_templateid, p_autobind):
    ...
    try:
        r = requests.post('https://%s/api/v0/networks/%s/bind' % (p_shardurl, p_nwid),
                          data=json.dumps({'configTemplateId': p_templateid, 'autoBind': autobindvalue}),
                          headers={'X-Cisco-Meraki-API-Key': p_apikey, 'Content-Type': 'application/json'})
    except:
...
def claimdeviceorg(p_apikey, p_shardurl, p_orgid, p_devserial):
    ...
    try:
        r = requests.post('https://%s/api/v0/organizations/%s/claim' % (p_shardurl, p_orgid),
                          data=json.dumps({'serial': p_devserial}),
                          headers={'X-Cisco-Meraki-API-Key': p_apikey, 'Content-Type': 'application/json'})
    except:
...
def claimlicenseorg(p_apikey, p_shardurl, p_orgid, p_licensekey):
    ...
    try:
        r = requests.post('https://%s/api/v0/organizations/%s/claim' % (p_shardurl, p_orgid),
                          data=json.dumps({'licenseKey': p_licensekey, 'licenseMode': 'addDevices'}),
                          headers={'X-Cisco-Meraki-API-Key': p_apikey, 'Content-Type': 'application/json'})
    except:
...
def claimdevice(p_apikey, p_shardurl, p_nwid, p_devserial):
    ...
    try:
        r = requests.post('https://%s/api/v0/networks/%s/devices/claim' % (p_shardurl, p_nwid),
                          data=json.dumps({'serial': p_devserial}),
                          headers={'X-Cisco-Meraki-API-Key': p_apikey, 'Content-Type': 'application/json'})
    except:
...
def getdeviceinfo(p_apikey, p_shardurl, p_nwid, p_serial):
    ...
    try:
        r = requests.get('https://%s/api/v0/networks/%s/devices/%s' % (p_shardurl, p_nwid, p_serial),
                         headers={'X-Cisco-Meraki-API-Key': p_apikey, 'Content-Type': 'application/json'})
    except:
...
def setdevicedata(p_apikey, p_shardurl, p_nwid, p_devserial, p_field, p_value, p_movemarker):
    ...
    try:
        r = requests.put('https://%s/api/v0/networks/%s/devices/%s' % (p_shardurl, p_nwid, p_devserial),
                         data=json.dumps({p_field: p_value, 'moveMapMarker': movevalue}),
                         headers={'X-Cisco-Meraki-API-Key': p_apikey, 'Content-Type': 'application/json'})
    except:
...
def getorgdeviceinfo(p_apikey, p_shardurl, p_orgid, p_devserial):
    ...
    try:
        r = requests.get('https://%s/api/v0/organizations/%s/inventory' % (p_shardurl, p_orgid),
                         headers={'X-Cisco-Meraki-API-Key': p_apikey, 'Content-Type': 'application/json'})
    except:
    ...
    for record in rjson:
        if record['serial'] == p_devserial:
            foundserial = True
            returnvalue = {'mac': record['mac'], 'serial': record['serial'], 'networkId': record['networkId'],
                           'model': record['model'], 'claimedAt': record['claimedAt'], 'publicIp': record['publicIp']}
...
def getgoogletimezone(p_googlekey, p_address):
    ...
    try:
        r = requests.get(
            'https://maps.googleapis.com/maps/api/geocode/json?address=%s&key=%s' % (p_address, p_googlekey))
    except:
    ...
    try:
        s = requests.get('https://maps.googleapis.com/maps/api/timezone/json?location=%s,%s&timestamp=%f&key=%s' % (
        glatitude, glongitude, time.time(), p_googlekey))
    except:
        printusertext('WARNING: Unable to contact Google cloud')
        return ('null')
...
def main(argv):
    ...
    nwparams = {'name': arg_nwname, 'timeZone': 'Europe/Helsinki', 'tags': nwtags, 'organizationId': orgid,
                'type': nwtypestring}
    ...
    validserials = []
    for i in range(0, len(devicelist['serial'])):
        if devicelist['model'][i][:2] == 'MR' or devicelist['model'][i][:2] == 'MS' or devicelist['model'][i][
                                                                                       :2] == 'MX' or \
                devicelist['model'][i][:1] == 'Z':
